use common::{Span, Attributes};
use oltp::opentelemetry::proto::trace;
use oltp::opentelemetry::proto::common::v1::{KeyValue, AnyValue};
use oltp::opentelemetry::proto::common::v1::any_value;
use serde_json::{Value};
use oltp::opentelemetry::proto::trace::v1::span::{Event, Link};
use oltp::opentelemetry::proto::trace::v1::{ResourceSpans, InstrumentationLibrarySpans};
...
pub fn serialize(spans: &[Span]) -> Result<Vec<u8>, EncodeError> {
    ...
    let resource_spans = ResourceSpans {
        ...
        instrumentation_library_spans: vec![
            InstrumentationLibrarySpans {
                ...
                spans: spans.iter().map(|span| trace::v1::Span {
                    ...
                    parent_span_id: span.parent_span_id.clone().unwrap_or("".into()).into_bytes(),
                    ...
                    events: span.events.as_ref().unwrap_or(&vec![]).iter().map(|evt|
                        Event {
                    ...
                    links: span.links.as_ref().unwrap_or(&vec![]).iter().map(|link|
                        Link {
    ...
    let elapse_time = Instant::now() - start;
    println!("Protocol buffer serialization: {}ms", elapse_time.as_millis());
...
fn attributes(attributes: Option<&Attributes>) -> Vec<KeyValue> {
    attributes.iter().flat_map(|attributes| attributes
        .iter()
        ...
        .map(|(key, value)|
            KeyValue {
                ...
                value: match value {
                    Value::Null => None,
                    Value::Bool(v) => Some(AnyValue { value: Some(any_value::Value::BoolValue(*v)) }),
                    ...
                    Value::String(v) => Some(AnyValue { value: Some(any_value::Value::StringValue(v.clone())) }),
