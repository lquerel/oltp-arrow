...
use arrow::array::{
    ArrayRef, StringArray, StringBuilder, UInt32Array, UInt32Builder, UInt64Array, UInt64Builder,
};
...
pub fn serialize_spans(spans: &[Span]) -> Result<Vec<u8>, ArrowError> {
    ...
    let mut columns: Vec<ArrayRef> = vec![
        Arc::new(UInt32Array::from_iter_values(0..spans.len() as u32)),
        Arc::new(UInt64Array::from_iter_values(
            spans.iter().map(|span| span.start_time_unix_nano),
        )),
        Arc::new(end_time_unix_nano.finish()),
        Arc::new(StringArray::from_iter_values(
            spans.iter().map(|span| span.trace_id.clone()),
        )),
        Arc::new(StringArray::from_iter_values(
            spans.iter().map(|span| span.span_id.clone()),
        )),
